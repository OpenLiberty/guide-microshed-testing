...
To do this, you imported the [hotspot=importInject file=0]`org.microshed.testing.jaxrs.RESTClient` annotation, 
created a [hotspot=personSvc file=0]`PersonService` REST client, and annotated the REST client with [hotspot=inject file=0]`@RESTClient`.
...
Remove the container code from the `PersonServiceIT` class by removing [hotspot=importMPApp hotspot=importContainer file=0]`import` statements and the [hotspot=container hotspot=mpApp file=0]`ApplicationContainer app` field.
...
Similarly, remove the container code from the `PersonServiceIT` class by removing [hotspot=importMPApp hotspot=importContainer file=0]`import` statements and the [hotspot=container hotspot=mpApp file=0]`ApplicationContainer app` field.
