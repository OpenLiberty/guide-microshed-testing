...
The `ErrorPathIT` test class has the same [hotspot=container file=2]`@Container` configuration and [hotspot=personSvc file=2]`PersonService` REST client as the `PersonServiceIT` class.
...
Remove [hotspot=importMPApp hotspot=importContainer file=4]`import` statements for `ApplicationContainer` and `Container` and the [hotspot=container hotspot=mpApp file=4]`ApplicationContainer app` field.
...
----
[role="edit_command_text"]
Import the [hotspot=importSharedContainerConfig file=5]`SharedContainerConfig` annotation and annotate the `PersonServiceIT` class with [hotspot=sharedContainerConfig file=5]`@SharedContainerConfig`. 
...
Remove [hotspot=importMPApp hotspot=importContainer file=6]`import` statements for `ApplicationContainer` and `Container` and the [hotspot=container hotspot=mpApp file=6]`ApplicationContainer app` field.
Next, annotate the `ErrorPathIT` class with the `@SharedContainerConfig` annotation.
[role="code_command hotspot file=7", subs="quotes"]
----
#Replace the `ErrorPathIT` class.#
`src/test/java/io/openliberty/guides/testing/ErrorPathIT.java`
----
[role="edit_command_text"]
Import the [hotspot=importSharedContainerConfig file=7]`SharedContainerConfig` annotation and annotate the `ErrorPathIT` class with [hotspot=sharedContainerConfig file=7]`@SharedContainerConfig`. 
