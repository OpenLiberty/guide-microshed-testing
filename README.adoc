...
First, look at `PersonServiceIT` class to see what the tests look like:
...
can make code changes to the application or tests in between re-running the tests. Development mode will automatically
...
PersonServiceIT.java
[source, java, linenums, role='code_column hide_tags=copyright,importInject,importAssertNotNull,importSharedContainerConfig,sharedContainerConfig,inject,personSvc,testCreatePerson,tests']
----
include::hotspots/src/test/java/io/openliberty/guides/testing/PersonServiceIT.2.java[]
----
...
PersonServiceIT.java
[source, java, linenums, role='code_column hide_tags=copyright,importAssertNotNull,importSharedContainerConfig,sharedContainerConfig,testCreatePerson,tests']
----
include::hotspots/src/test/java/io/openliberty/guides/testing/PersonServiceIT.2.java[]
----
...
PersonServiceIT.java
[source, java, linenums, role='code_column hide_tags=copyright,importSharedContainerConfig,sharedContainerConfig,tests']
----
include::hotspots/src/test/java/io/openliberty/guides/testing/PersonServiceIT.2.java[]
----
...
----
[role="edit_command_text"]
Add tests [hotspot=testMinSizeName file=1]`testMinSizeName()`, [hotspot=testMinAge file=1]`testMinAge()`, [hotspot=testGetPerson file=1]`testGetPerson()`, [hotspot=testGetAllPeople file=1]`testGetAllPeople()`, and [hotspot=testUpdateAge file=1]`testUpdateAge()`.
...
Then, remove the container code in `PersonServiceIT` class, and annotate `@SharedContainerConfiguration` referencing the shared configuration class.
...
----
[role="edit_command_text"]
Remove [hotspot=importMPApp hotspot=importContainer file=2]`import` statements and [hotspot=container hotspot=mpApp file=2]`MicroProfileApplication app` field, import [hotspot=importSharedContainerConfig file=2]`SharedContainerConfig` annotation, and annotate the `PersonServiceIT` class with [hotspot=sharedContainerConfig file=2]`@SharedContainerConfig`. 
...
The [hotspot file=3]`PersonServiceIT` class should be same as the code in the code pane .
...
Then, update the `ErrorPathIT` class samiliarly.
...
----
[role="edit_command_text"]
Remove [hotspot=importMPApp hotspot=importContainer file=4]`import` statements and [hotspot=container hotspot=mpApp file=4]`MicroProfileApplication app` field, import [hotspot=importSharedContainerConfig file=4]`SharedContainerConfig` annotation, and annotate the `ErrorPathIT` class with [hotspot=sharedContainerConfig file=4]`@SharedContainerConfig`. 
...
The [hotspot file=5]`ErrorPathIT` class should be same as the code in the code pane .
