...
Replace the `PersonServiceIT` class to include the [hotspot=importAssertNotNull file=1]`assertNotNull` static method and write the test logic in the [hotspot=testCreatePerson file=1]`testCreatePerson()` method.
...
The following tests are added: [hotspot=testMinSizeName file=2]`testMinSizeName()`, [hotspot=testMinAge file=2]`testMinAge()`, [hotspot=testGetPerson file=2]`testGetPerson()`, [hotspot=testGetAllPeople file=2]`testGetAllPeople()`, and [hotspot=testUpdateAge file=2]`testUpdateAge()`.
...
Remove [hotspot=importMPApp hotspot=importContainer file=2]`import` statements for `ApplicationContainer` and `Container` and the [hotspot=container hotspot=mpApp file=2]`ApplicationContainer app` field.
