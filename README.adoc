...
Now that the setup is complete, you can write your first test cases. Start by testing the basic "create person" use case for your REST-based application. To test this use case, use the REST client that's injected by MicroShed Testing to make the HTTP POST request to the application and read the response.
Next, add additional tests to broaden coverage.
...
The following tests are then added: [hotspot=testMinSizeName file=0]`testMinSizeName()`, [hotspot=testMinAge file=0]`testMinAge()`, [hotspot=testGetPerson file=0]`testGetPerson()`, [hotspot=testGetAllPeople file=0]`testGetAllPeople()`, and [hotspot=testUpdateAge file=0]`testUpdateAge()`.
...
Remove the container code from the `PersonServiceIT` class, and annotate the `PersonServiceIT` class with the `@SharedContainerConfig` annotation that references the `AppDeploymentConfig` shared configuration class.
...
Similarly, replace the `ErrorPathIT` class to remove the container code and annotate the `ErrorPathIT` class with the `@SharedContainerConfig` annotation.
...
Import the [hotspot=importSharedContainerConfig file=4]`SharedContainerConfig` annotation and annotate the `ErrorPathIT` class with [hotspot=sharedContainerConfig file=4]`@SharedContainerConfig`. 
